...
// 현재가 (Ticker)
export type UpbitWebSocketTicker = {
  type: string; //									  타입 - ticker : 현재가
  ...
  isPriceChanged?: boolean; //추가
};


export type UpbitCoinInfo = {
  market: string;//									  타입 - ticker : 현재가
  trade_date: string;
  trade_time: string;
  trade_date_kst: string;
  trade_time_kst: string;
